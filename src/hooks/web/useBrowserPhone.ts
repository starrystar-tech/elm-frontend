...
const appendLog = (
  payload: Pick<BrowserPhoneLogItem, 'caller' | 'callee' | 'message' | 'jobUuid' | 'label' | 'type'>
) => {
  logs.value.unshift({
...
const showIncomingNotification = (caller: string) => {
  ...
  if (Notification.permission !== 'granted') {
    traceBrowserStep('NOTIFICATION_SKIP', `通知权限不是 granted，而是 ${Notification.permission}`, 'danger')
    return
  }
  try {
  ...
  const client = new SimpleUser(browserForm.wsServer, {
    ...
    delegate: {
      ...
      onUnregistered: () => {
        ...
        traceBrowserStep('UNREGISTERED', `connecting=${browserConnecting.value}, disconnecting=${browserDisconnecting.value}, reason=${reason}`)
        if (!browserDisconnecting.value && !browserConnecting.value) addBrowserLog('浏览器分机已注销')
      },
      onServerConnect: () => {
  ...
  if (sessionManager) {
    sessionManager.delegate = {
      ...
      onCallAnswered: () => {
        ...
        stopIncomingRing()
        startCallTimer()
        void syncBrowserRecord({ recordId: browserRecordId.value, event: 'answered', caller, callee })
...
const connectBrowserPhone = async () => {
  ...
  try {
    ...
    const registerRequest = client.register({
      requestDelegate: {
        onTrying: (response: any) => traceBrowserStep('REGISTER_TRYING', describeSipResponse(response)),
        onProgress: (response: any) => traceBrowserStep('REGISTER_PROGRESS', describeSipResponse(response)),
        onAccept: (response: any) => traceBrowserStep('REGISTER_ACCEPT', describeSipResponse(response)),
        onReject: (response: any) => traceBrowserStep('REGISTER_REJECT', describeSipResponse(response), 'danger')
      }
    })
    ...
    traceBrowserStep('CONNECT_FAILED', `${errorMessage}; ${describeBrowserError(error)}`, 'danger')
...
const disconnectBrowserPhone = async (silent = false) => {
  ...
  if (client) {
    try { await client.hangup?.().catch(() => undefined) } catch {}
    if (wasRegistered) {
      try { await client.unregister?.().catch(() => undefined) } catch {}
    }
    try { await client.disconnect?.().catch(() => undefined) } catch {}
  }
  browserRegistered.value = false
  incomingCall.value = false
  activeCall.value = false
...
const makeBrowserCall = async () => {
  ...
  try {
    ...
    await browserClient.value.call(`sip:${target}@${browserForm.domain.trim()}`, {
      extraHeaders: browserRecordId.value ? [`X-CRM-Record-Id: ${browserRecordId.value}`] : undefined
    })
...
const answerBrowserCall = async () => {
  ...
  try {
    ...
    traceBrowserStep('ANSWER_FAILED', `${errorMessage}; ${describeBrowserError(error)}`, 'danger')
    addBrowserLog(errorMessage, '失败', 'danger')
    message.error(errorMessage)
  }
}
