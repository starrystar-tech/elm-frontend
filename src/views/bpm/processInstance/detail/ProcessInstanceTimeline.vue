        ...
        >
            <template #dot>
                ...
                >
                    <img
                        class="w-full h-full"
                        :src="getApprovalNodeImg(activity.nodeType)"
                        alt=""
                    />
                    <div
                    ...
                    >
                        <el-icon :size="11" color="#fff">
                            <component
                                :is="getApprovalNodeIcon(activity.status, activity.nodeType)"
                            />
                        </el-icon>
                    </div>
                </div>
                ...
                <div v-else class="flex items-center flex-wrap mt-1 gap2">
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                <template
                                    v-if="task.assigneeUser?.avatar || task.assigneeUser?.nickname"
                                >
                                    <el-avatar
                                        class="!m-5px"
                                        :size="28"
                        ...
                        <teleport defer :to="`#activity-task-${activity.id}-${index}`">
                            <div
                                v-if="
                                    task.reason &&
                                    [NodeType.USER_TASK_NODE, NodeType.END_EVENT_NODE].includes(
                                        activity.nodeType
                                    )
                                "
                            ...
                            <div
                                v-if="
                                    task.signPicUrl && activity.nodeType === NodeType.USER_TASK_NODE
                                "
                                class="text-#a5a5a5 text-13px mt-1 w-full bg-#f8f8fa p2 rounded-md"
                            >
                                签名：
...
const handleSelectUser = (activityId, selectedList) => {
    userSelectFormRef.value.open(activityId, selectedList, { multiple: true })
}
