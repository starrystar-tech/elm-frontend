<template>
  <el-form ref="repurchaseFormRef" v-loading="repurchaseLoading" :model="repurchaseForm" :rules="repurchaseRules" label-width="160px">
    <el-card shadow="never">
      <template #header>
        <div class="flex items-center justify-between">
          <CardTitle title="复购公海规则设置" />
          <el-button type="primary" v-hasPermi="['crm:customer-pool-config:update']" @click="saveRepurchaseConfig">
            保存
          </el-button>
        </div>
      </template>
      ...
      <el-form-item label="是否启用" prop="enabled">
        <el-radio-group v-model="repurchaseForm.enabled" @change="changeRepurchaseEnable" class="ml-4">
          <el-radio :value="false" size="large">不启用</el-radio>
          <el-radio :value="true" size="large">启用</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item label="是否优先新数据分配" prop="priorityNewDataAllocation">
        <el-radio-group v-model="repurchaseForm.priorityNewDataAllocation" class="ml-4">
      ...
      <template v-if="repurchaseForm.enabled">
        <el-form-item label="生效时间范围" prop="timeRangeType">
          <el-radio-group v-model="repurchaseForm.timeRangeType" @change="changeRepurchaseTimeRangeType">
            <el-radio value="unlimited">不限</el-radio>
            <el-radio value="custom">自定义</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item v-if="repurchaseForm.timeRangeType === 'custom'" label="生效时间">
          <el-time-picker v-model="repurchaseForm.startTime" value-format="HH:mm:ss" placeholder="开始时间" />
          <span class="mx-2">至</span>
          <el-time-picker v-model="repurchaseForm.endTime" value-format="HH:mm:ss" placeholder="结束时间" />
        </el-form-item>

        <el-form-item label="报名商品范围" prop="productScopeType">
          <el-radio-group v-model="repurchaseForm.productScopeType" @change="changeRepurchaseProductScopeType">
            <el-radio value="unlimited">不限</el-radio>
            <el-radio value="custom">自定义</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item v-if="repurchaseForm.productScopeType === 'custom'" label="报名商品">
          <el-select v-model="repurchaseForm.productIdList" multiple clearable collapse-tags collapse-tags-tooltip placeholder="请选择商品">
        ...
        <el-form-item v-if="repurchaseForm.dealTimeRangeType === 'custom'" label="成单时间">
          <el-date-picker v-model="repurchaseForm.dealStartDate" type="date" value-format="YYYY-MM-DD" placeholder="开始日期" />
          <span class="mx-2">至</span>
          <el-date-picker v-model="repurchaseForm.dealEndDate" type="date" value-format="YYYY-MM-DD" placeholder="结束日期" />
        </el-form-item>
        ...
        <el-form-item v-if="repurchaseForm.signTimeRangeType === 'custom'" label="报名时间">
          <el-date-picker v-model="repurchaseForm.signStartDate" type="date" value-format="YYYY-MM-DD" placeholder="开始日期" />
          <span class="mx-2">至</span>
          <el-date-picker v-model="repurchaseForm.signEndDate" type="date" value-format="YYYY-MM-DD" placeholder="结束日期" />
        </el-form-item>
        ...
        <el-form-item v-if="repurchaseForm.paymentAmountType === 'proportion'" label="付款比例">
          <el-input-number v-model="repurchaseForm.paymentPercentMin" :min="0" :max="100" />
          <span class="mx-2">至</span>
          <el-input-number v-model="repurchaseForm.paymentPercentMax" :min="0" :max="100" />
        </el-form-item>
...
const repurchaseRules = reactive({
  ...
  priorityNewDataAllocation: [{ required: true, message: '是否优先新数据分配不能为空', trigger: 'change' }],
