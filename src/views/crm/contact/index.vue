...
const searchSchema = reactive<FormSchema[]>([
  ...
  },
  {
    ...
    label: '电子邮箱',
    component: 'Input',
    componentProps: { placeholder: '请输入电子邮箱', clearable: true, style: { width: '240px' } }
  }
])
...
const { tableObject, tableMethods, register: tableRegister } = useTable<ContactApi.ContactVO>({
...
const tableColumns = reactive<TableColumn[]>([
  ...
  },
  {
    ...
    slots: {
      default: (data) =>
        data.row.parentId ? (
          <ElLink underline={false} type="primary" onClick={() => openDetail(data.row.parentId)}>
            {data.row.parentName}
          </ElLink>
        ) : (
          <>{data.row.parentName}</>
