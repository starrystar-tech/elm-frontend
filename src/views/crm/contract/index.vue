...
const searchSchema = computed<FormSchema[]>(() => [
  ...
  },
  {
    ...
    label: '合同名称',
    component: 'Input',
    componentProps: { placeholder: '请输入合同名称', clearable: true, style: { width: '240px' } }
  },
  {
    field: 'customerId',
    label: '客户',
...
const { tableObject, tableMethods, register: tableRegister } = useTable<ContractApi.ContractVO>({
...
const tableColumns: TableColumn[] = [
  ...
  },
  {
    ...
    slots: {
      default: (data) =>
        data.row.businessId ? (
          <ElLink underline={false} type="primary" onClick={() => openBusinessDetail(data.row.businessId)}>
  ...
  { field: 'totalPrice', label: '合同金额（元）', width: '140px', formatter: erpPriceTableColumnFormatter },
    ...
    slots: {
      default: (data) =>
        data.row.signContactId ? (
          <ElLink underline={false} type="primary" onClick={() => openContactDetail(data.row.signContactId)}>
  ...
  },
  {
    ...
    width: '140px',
    slots: {
      default: (data) => <>{erpPriceInputFormatter(data.row.totalPrice - data.row.totalReceivablePrice)}</>
    }
  },
