<template>
    <div class="contract-records">
        <el-table v-if="contractList.length" v-loading="loading" :data="contractList" border>
        ...
        <el-empty v-else v-loading="loading" description="暂无合同信息" :image-size="60" />
        ...
        >
            <div v-loading="signUrlLoading" class="contract-sign-url">
                ...
                <el-input v-model="signUrl" readonly>
                    <template #append>
                        <el-button :disabled="!signUrl" @click="handleCopySignUrl">
                            复制链接
                        </el-button>
                    </template>
                </el-input>
...
const canUpdateContract = hasPermission(['system:contract:update'])
...
const loadData = async () => {
    ...
    loading.value = true
    try {
        contractList.value = await ContractApi.getContractListByClue(Number(props.clueId))
    } finally {
        loading.value = false
    }
}
...
const canAbolish = (row: ContractApi.ContractPageRespVO) =>
    canUpdateContract && Boolean(row.contractNo) && Number(row.status) !== 5
