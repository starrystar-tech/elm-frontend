<template>
  ...
  <StockInForm ref="formRef" @success="tableMethods.getList" />
</template>
...
const canCreate = hasPermission(['erp:stock-in:create'])
const canQuery = hasPermission(['erp:stock-in:query'])
const canUpdate = hasPermission(['erp:stock-in:update'])
const canUpdateStatus = hasPermission(['erp:stock-in:update-status'])
const canDelete = hasPermission(['erp:stock-in:delete'])
const canExport = hasPermission(['erp:stock-in:export'])
...
const { tableObject, tableMethods, register: tableRegister } = useTable<StockInVO>({
  getListApi: async (params) => await StockInApi.getStockInPage(params),
  exportListApi: async (params) => await StockInApi.exportStockIn(params)
})
...
const handleExport = async () => {
  await tableMethods.exportList('其它入库单.xls')
}
