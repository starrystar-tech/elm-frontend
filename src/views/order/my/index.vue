...
const searchSchema = computed<FormSchema[]>(() => [
    ...
    { field: 'orderStatus', label: '订单状态', component: 'Select', componentProps: { options: ORDER_STATUS_OPTIONS, clearable: true, style: { width: '220px' } } },
    { field: 'enrollTimeRange', label: '报名时间', component: 'DatePicker', componentProps: { type: 'daterange', valueFormat: 'YYYY-MM-DD HH:mm:ss', style: { width: '260px' } } },
    { field: 'contractStatus', label: '合同签署', component: 'Select', componentProps: { options: CONTRACT_STATUS_OPTIONS, clearable: true, style: { width: '220px' } } },
    { field: 'productCategory', label: '商品分类', component: 'Input', componentProps: { clearable: true, style: { width: '220px' } } },
    { field: 'productKeyword', label: '商品', component: 'Input', componentProps: { clearable: true, style: { width: '220px' } } },
    { field: 'minPaidAmount', label: '已付金额起', component: 'Input', componentProps: { clearable: true, style: { width: '160px' } } },
    { field: 'maxPaidAmount', label: '已付金额止', component: 'Input', componentProps: { clearable: true, style: { width: '160px' } } },
    { field: 'ownerUserName', label: '订单归属', component: 'Input', componentProps: { clearable: true, style: { width: '220px' } } },
    { field: 'cardOwnerUserName', label: '名片归属', component: 'Input', componentProps: { clearable: true, style: { width: '220px' } } },
    { field: 'expireTimeRange', label: '到期时间', component: 'DatePicker', componentProps: { type: 'daterange', valueFormat: 'YYYY-MM-DD', style: { width: '240px' } } },
...
const {
    ...
    getListApi: async (params) => await OrderApi.getMyOrderPage(params)
})
...
const openDetail = (id: number, tab?: string) => {
    router.push({ name: 'OrderDetail', params: { id }, query: tab ? { tab } : undefined })
}
...
const handlePay = async (row: OrderApi.OrderPageRespVO) => {
    ...
    await OrderApi.payOrder({ orderId: row.id, payAmount: Number(result.value), payMethod: '微信支付' })
...
const tableColumns = computed<TableColumn[]>(() => [
    ...
    { field: 'payableAmount', label: '应付金额', minWidth: '100px', formatter: (_r, _c, v) => formatAmount(v) },
    { field: 'paidAmount', label: '已付金额', minWidth: '100px', formatter: (_r, _c, v) => formatAmount(v) },
    { field: 'refundAmount', label: '已退费金额', minWidth: '110px', formatter: (_r, _c, v) => formatAmount(v) },
