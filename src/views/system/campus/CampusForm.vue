<template>
    <Dialog v-model="dialogVisible" :title="dialogTitle" width="680px">
        ...
        >
            ...
            <el-form-item prop="contractSignCompanyId">
                <template #label>
                    <span class="inline-flex items-center gap-4px">
                        <span>签约公司</span>
                        <el-tooltip content="用于该分校电子合同指定签约公司" placement="top">
                            <Icon icon="ep:question-filled" class="text-14px text-[#909399]" />
                        </el-tooltip>
                    </span>
                </template>
                <el-select
...
const open = async (type: string, id?: number) => {
    ...
    dialogTitle.value = t('action.' + type) + '校区'
    formType.value = type
    resetForm()
