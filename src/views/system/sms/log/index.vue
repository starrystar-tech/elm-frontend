...
const { tableObject, tableMethods, register: tableRegister } = useTable<SmsLogApi.SmsLogVO>({
...
const tableColumns = reactive<TableColumn[]>([
  ...
  },
  {
    ...
    slots: {
      default: (data) => (
        <>
          <DictTag type={DICT_TYPE.SYSTEM_SMS_RECEIVE_STATUS} value={data.row.receiveStatus} />
  ...
  },
  {
    ...
    slots: {
      default: (data) => (
        <>
          <div>{getChannelName(data.row.channelId)}</div>
          <DictTag type={DICT_TYPE.SYSTEM_SMS_CHANNEL_CODE} value={data.row.channelCode} />
        </>
      )
    }
  },
  { field: 'templateId', label: '模板编号' },
  {
    ...
    label: '短信类型',
    slots: {
      default: (data) => <DictTag type={DICT_TYPE.SYSTEM_SMS_TEMPLATE_TYPE} value={data.row.templateType} />
    }
  },
  {
    field: 'action',
    ...
    slots: {
      default: (data) =>
        canQuery ? (
          <BaseButton link type="primary" onClick={() => openDetail(data.row as SmsLogApi.SmsLogVO)}>
            详情
          </BaseButton>
        ) : null
    }
